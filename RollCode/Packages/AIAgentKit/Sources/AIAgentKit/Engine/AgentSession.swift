...
final class AgentSession {
    ...
    func loadThreads(for workspaceURL: URL) {
        ...
        for var thread in loadedThreads {
            if thread.provider == .codex || thread.codexThreadID != nil || (thread.model?.hasPrefix("gpt") == true) {
    ...
    private func archiveChannel(_ channel: inout ProviderChannelState) {
        guard !channel.activeThread.entries.isEmpty || channel.activeThread.codexThreadID != nil else { return }
        channel.activeThread.provider = channel.provider
        if let index = channel.threads.firstIndex(where: { $0.id == channel.activeThread.id }) {
            channel.threads[index] = channel.activeThread
        } else {
            channel.threads.insert(channel.activeThread, at: 0)
        }
    }

    private func archiveCurrentThreadIfNeeded() {
        archiveChannel(&codexChannel)
        archiveChannel(&geminiChannel)
    }
    ...
    func makeContextualPrompt(_ prompt: String, activeFileURL: URL?) -> String {
        ...
        // For Gemini (stateless CLI calls), inject past conversation history from the Gemini active thread
