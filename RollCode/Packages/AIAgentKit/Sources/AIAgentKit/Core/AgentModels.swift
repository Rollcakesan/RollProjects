...
public struct AgentThread: Identifiable, Equatable, Sendable, Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        let decodedProvider = try container.decodeIfPresent(AgentProvider.self, forKey: .provider) ?? .codex
        ...
        self.provider = (self.codexThreadID != nil || self.model?.hasPrefix("gpt") == true || self.model?.hasPrefix("o") == true) ? .codex : decodedProvider
    }
}
